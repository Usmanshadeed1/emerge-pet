...
const TABS: { key: Tab; label: string; icon: React.ReactNode }[] = [
  ...
  { key: "breed",     label: "Breed Guide", icon: <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" /></svg> },
...
export default function PetDetailClient({ pet, isPremium }: { pet: Pet; isPremium: boolean }) {
  ...
  return (
    <div className="mx-auto max-w-3xl anim-fade-in">
      ...
      {/* ── Hero Card ── */}
      <div className="mb-4 rounded-2xl bg-white dark:bg-gray-900 p-6 shadow-sm ring-1 ring-gray-200 dark:ring-gray-700">
        <div className="flex items-start gap-5">
          {/* Photo */}
          <div className="relative flex h-24 w-24 flex-shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-green-50 dark:bg-green-900/20 text-5xl ring-2 ring-green-100 dark:ring-green-900 shadow-md">
            {pet.hasPhoto
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <Link href={`/dashboard/pets/${pet.id}/edit`}
                  className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 text-sm font-medium text-gray-700 dark:text-gray-300 shadow-sm hover:bg-gray-50 dark:bg-gray-800 transition-all">
                ...
                <button onClick={() => setShowDelete(true)}
                  className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-3 text-sm font-medium text-red-700 dark:text-red-400 hover:bg-red-100 transition-all">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {age    && <span className="inline-flex items-center gap-1.5 rounded-lg bg-green-50 dark:bg-green-900/20 px-2.5 py-1 text-xs font-medium text-green-700 dark:text-green-400 ring-1 ring-green-100 dark:ring-green-900">🎂 {age}</span>}
              {weight && <span className="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 px-2.5 py-1 text-xs font-medium text-blue-700 ring-1 ring-blue-100">⚖️ {weight}</span>}
              {pet.sex && <span className="inline-flex items-center gap-1.5 rounded-lg bg-purple-50 dark:bg-purple-900/20 px-2.5 py-1 text-xs font-medium text-purple-700 ring-1 ring-purple-100">{pet.sex === "MALE" ? "♂ Male" : "♀ Female"}</span>}
              {pet.isNeutered && <span className="inline-flex items-center rounded-lg bg-orange-50 px-2.5 py-1 text-xs font-medium text-orange-700 ring-1 ring-orange-100">✂️ Neutered</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ── Tabs ── */}
      <div className="mb-4 flex gap-1 overflow-x-auto rounded-xl bg-white dark:bg-gray-900 p-1 shadow-sm ring-1 ring-gray-200 dark:ring-gray-700 scrollbar-hide">
        {TABS.map(({ key, label, icon }) => (
          <button key={key} onClick={() => setTab(key)}
            className={`flex flex-shrink-0 items-center gap-1.5 rounded-lg px-3.5 py-2 text-sm font-medium transition-all ${
              tab === key
                ? "bg-green-600 text-white shadow-sm"
                : "text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:bg-gray-700 hover:text-gray-700 dark:text-gray-300"
            }`}>
            {icon} {label}
          </button>
        ))}
